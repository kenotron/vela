...
from vela_agentd_lib.persistence import workspaces_root
from vela_agentd_lib.protocol_points.defaults_http import (
    HttpAutoApprovalSystem,
    HttpQueueDisplaySystem,
)
from vela_agentd_lib.session_store import SessionStore

from vela_agentd_http._auth import require_bearer
from vela_agentd_http._c2_shapes import to_c2_payload
from vela_agentd_http._event_translator import extract_usage, translate_event
from vela_agentd_http._host_tool_signal import HostToolYield
from vela_agentd_http._reconciler import reconcile_client_history
from vela_agentd_http._session_runner import run_chat_turn
from vela_agentd_http._wire import (
...
logger = logging.getLogger("vela_agentd_http.chat_completions")
...
async def _drain_tee_to_c2(tee_queue: asyncio.Queue[Any], broadcaster: Any) -> None:
    """F3 second consumer: drain the tee queue, shape events per §4.2, publish to C2.

    Mirrors ``_drain_tee_to_jsonl``'s sentinel-terminated drain loop, reading
    from its OWN queue so it can never perturb C1 or the JSONL tee. Events not
    part of the C2 contract (``to_c2_payload`` returns None) are dropped --
    the SSE surface stays a clean, documented contract rather than a raw
    firehose of internal kernel event kinds.
    """
    while True:
        event = await tee_queue.get()
        if event is None:
            break
        try:
            shaped = to_c2_payload(event)
        except Exception:
            logger.debug("_drain_tee_to_c2: failed to shape event; dropping")
            continue
        if shaped is None:
            continue
        try:
            broadcaster.publish(shaped)
        except Exception:
            logger.debug("_drain_tee_to_c2: broadcaster.publish failed; dropping")
...
async def _stream_chat_completion(
    *,
    ...
    c2_tee_task: asyncio.Task[None] | None = None,
    ...
    try:
        ...
        # F3: same drain-then-cancel discipline for the C2 broadcaster tee.
        if c2_tee_task is not None:
            try:
                await asyncio.wait_for(c2_tee_task, timeout=1.0)
            except (TimeoutError, asyncio.CancelledError):
                if not c2_tee_task.done():
                    c2_tee_task.cancel()
                await asyncio.gather(c2_tee_task, return_exceptions=True)
...
async def chat_completions(
    ...
    # Fork point (F3): also tee raw kernel events to the C2 broadcaster so
    # connected SSE clients (GET /v1/events) see tool/started, tool/completed,
    # progress, thinking/*, usage, error -- shaped per design doc §4.2. Uses
    # the SAME tee_queue mechanism proven in S-1 (a second bounded queue is
    # created here specifically for the broadcaster consumer, independent of
    # the optional JSONL tee above, so neither can perturb the other or C1).
    c2_broadcaster = getattr(request.app.state, "c2_broadcaster", None)
    c2_tee_queue: asyncio.Queue[Any] | None = None
    c2_tee_task: asyncio.Task[None] | None = None
    if c2_broadcaster is not None:
        c2_tee_queue = asyncio.Queue(maxsize=_TEE_QUEUE_MAXSIZE)
        c2_tee_task = asyncio.create_task(_drain_tee_to_c2(c2_tee_queue, c2_broadcaster))
        if tee_queue is None:
            # No JSONL tee configured -- use this queue as the sole tee sink.
            tee_queue = c2_tee_queue
        # else: HttpQueueDisplaySystem only fans out to ONE tee_queue. When
        # both the JSONL tee and the C2 broadcaster are active we chain the
        # JSONL drain to also forward to the broadcaster (see below) rather
        # than trying to attach two tee queues to one DisplaySystem.

    display = HttpQueueDisplaySystem(event_queue, tee_queue=tee_queue)

    # Fork point (F2): replace unconditional auto-approve with a real gate
    # that suspends on a privileged tool call, publishes on C2, and resolves
    # via a C2 client decision or a bounded timeout defaulting to deny.
    approval_gate = getattr(request.app.state, "approval_gate", None)
    approval: Any = approval_gate if approval_gate is not None else HttpAutoApprovalSystem()
