...
from vela_agentd_cli.admin.models import (
    ProviderCredentialsMissingError,
    ProviderModuleNotInstalledError,
    list_provider_models,
)
from vela_agentd_cli.provider_sources import PROVIDER_CATALOG
from vela_agentd_lib._runtime import prepare_bundle_for_session
from vela_agentd_lib.bundle.cache import load_and_prepare_cached
from vela_agentd_lib.config import ConfigError
from vela_agentd_lib.config import load_config as load_host_config
from vela_agentd_lib.persistence import resolve_workspace
...
def _resolve_aaa_version() -> str:
    ...
    Mirrors the pattern used inside ``vela_agentd_lib.__init__``. We do it
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # Fork point (F3/F2/F4): process-wide C2 broadcaster, approval gate, and
    # ledger proxy client. These are new additions (not modifications of
    # upstream logic) -- registered on app.state so route handlers and the
    # per-request chat_completions turn can reach them.
    from vela_agentd_http._approval_gate import ApprovalGate

    app.state.c2_broadcaster = EventBroadcaster()
    app.state.approval_gate = ApprovalGate(broadcaster=app.state.c2_broadcaster)
    app.state.ledger_client = LedgerProxyClient()
    ...
    from vela_agentd_cli.admin.serve_lifecycle import (
        remove_state_file,
        write_state_file,
    )
...
def build_app() -> FastAPI:
    ...
    # Fork additions: F3 (C2 event route), F2 (approval decision control route),
    # F4 (C3 ledger proxy routes). New routers, not modifications of upstream ones.
...
# Module-level app for `uvicorn vela_agentd_http.app:app`.
app = build_app()
