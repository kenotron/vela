...
async def _check_hooks() -> None:
    ...
    Imports ``vela_agentd_lib.bundle.hook_streaming`` and checks that
    ...
    """
    try:
        streaming_mod = importlib.import_module("vela_agentd_lib.bundle.hook_streaming")
    except ImportError as exc:
        click.echo(f"[FAIL] vela_agentd_lib.bundle.hook_streaming not importable: {exc}")
        sys.exit(1)
