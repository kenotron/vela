...
from vela_agentd_lib import persistence
from vela_agentd_lib.bundle import BUNDLE_MD
from vela_agentd_lib.bundle.loader import load_and_prepare_bundle
...
def cache_dir_for_version(aaa_version: str, bundle_path: Path | None = None) -> Path:
    ...
    The XDG cache root is owned by :func:`vela_agentd_lib.persistence.cache_root` —
    ...
    Args:
        ...
        bundle_path: Path to the bundle manifest file whose content contributes to the
            cache key.  Defaults to the vendored :data:`~vela_agentd_lib.bundle.BUNDLE_MD`
            when ``None``.
...
async def load_and_prepare_cached(aaa_version: str) -> PreparedBundle:
    ...
    :func:`~vela_agentd_lib.bundle.loader.load_and_prepare_bundle`.  A corrupted
    pickle triggers a warning log, removes both stale files, and falls through to the
    cold path.

    Cold path: calls
    :func:`~vela_agentd_lib.bundle.loader.load_and_prepare_bundle`, writes the
