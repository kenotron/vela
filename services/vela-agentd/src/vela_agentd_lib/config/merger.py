...
returned by :func:`vela_agentd_lib.config.load_config` when neither
...
separate return field so :mod:`vela_agentd_lib._runtime.engine.boot` can
...
def merge_config(
    *,
    ...
    :param host_config: The parsed host config dict from
        :func:`vela_agentd_lib.config.load_config`, or ``None`` when
