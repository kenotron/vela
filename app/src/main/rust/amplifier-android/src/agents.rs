...
pub fn build_agent_registry(vault_path: &Path) -> Arc<AgentRegistry> {
    ...
    // Register all built-in foundation agents.
...
mod tests {
    ...
    fn build_registry_includes_six_foundation_agents() {
        ...
        assert!(
            names.contains(&"security-guardian"),
    ...
    fn build_registry_missing_vault_dir_is_not_an_error() {
        let registry =
            build_agent_registry(Path::new("/definitely/does/not/exist/vault"));
        assert_eq!(
            registry.available_names().len(),
            6,
            "expected exactly 6 foundation agents"
        );
    }
}
