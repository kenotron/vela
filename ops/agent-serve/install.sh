...
if [[ ! -f "${ENV_FILE}" ]]; then
  ...
  cat > "${ENV_FILE}" <<EOF
# Environment for vela-agent-serve.service — DO NOT COMMIT (lives outside repo).
AMPLIFIER_AGENT_HTTP_API_KEY=${API_KEY}
AMPLIFIER_AGENT_HTTP_WORKSPACE=vela-lane-1-4
...
systemctl --user enable --now vela-agent-serve.service
...
echo ""
echo "Run ops/agent-serve/health-check.sh to verify."
