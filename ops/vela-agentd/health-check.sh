#!/usr/bin/env bash
# Health check for vela-agentd.service.
#
# Usage: ops/vela-agentd/health-check.sh [--port PORT] [--key API_KEY]
#
# Same contract as ops/agent-serve/health-check.sh (drop-in replacement,
# lane 3.1): port 9099, unauthenticated GET /v1/models expects 401,
# authenticated GET /v1/models expects 200.
#
# Exit codes:
...
ENV_FILE="${HOME}/.amplifier/vela-agentd/env"
...
if ! systemctl --user is-active --quiet vela-agentd.service; then
  echo "FAIL: vela-agentd.service is not active"
...
echo "== C2 SSE route reachable (expect 401 unauthenticated) =="
C2_CODE=$(curl -s -o /dev/null -w "%{http_code}" --max-time 3 "${BASE_URL}/v1/events" || echo "000")
if [[ "$C2_CODE" != "401" ]]; then
  echo "WARN: /v1/events returned ${C2_CODE} unauthenticated, expected 401 (non-fatal, informational)"
else
  echo "OK: /v1/events enforces auth (401)"
fi
